...
export class RegisterRetailerComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    this.subscribe = this.retailerStore
      .registerNewRetailer(newRetailer)
      .subscribe(
        (response) => {
          if (response.success) {
            this.openSnackBar(
              'Creación de cuenta exitosa! Ingrese como usuario vendedor.',
              ''
            );
            this.loading = false;
            ...
            if (response.data.code == 11000) {
              this.errorMessage =
                'El correo ingresado ya existe. Intente con un nuevo correo.';
              this.loading = false;
            }
          }
        },
