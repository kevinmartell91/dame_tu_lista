...
@Injectable()
export class BuyerAccountStore
  extends Store<BuyerAccountStoreState>
  implements OnDestroy
{
  ...
  private initReloadBuyerOrigin$(buyer_id: string): void {
    this.reloadBuyer$
      .pipe(
        switchMap(() => {
          return this.endPoint.getBuyerAccount(
            this.storeRequestUpdater,
            buyer_id
          );
        }),
  ...
  private initReloadBuyer$(buyer_id: string) {
    return this.endPoint
      .getBuyerAccount(this.storeRequestUpdater, buyer_id)
      .pipe(
        map((data: any) => {
          this.setState({
            ...this.state,
  ...
  public addFavoriteReatailer(buyer_id: string, retailer_email: string) {
    return this.endPoint
      .patchtFavoriteRetailer(
        buyer_id,
        retailer_email,
        this.storeRequestUpdater
      )
