...
var airtableController = require("../controllers/airtable");
...
var authorizationMiddleware = require("../routes/simple_jwt")
...
router.route("/authenticate").post(authenticationController.postAuthenticate);

// Create endpoint handlers for /buyers/:buyer_id
router
  .route("/buyers/:buyer_id")
...
router
  .route("/retailer-store/:retailer_id")
  .put(retailerController.putRetailerStore);

router
  .route("/retailer-product-list/:retailer_id")
  .post(retailerController.postRetailerProductList)
  .put(retailerController.putRetailerProductList);

router
  .route("/dtl-airtable-mongo/:retailer_id")
  .get(airtableController.getAirtableRecords);
...
router
  .route("/orders-by-retailer-id/:retailer_id")
...
router
  .route("/delete-all-orders/:retailer_id")
  .delete(orderController.deleteAllOrdersByReatailerId);

router
  .route("/:retailer_store_name")
